...
import { maintenanceMockData } from '../mock/maintenance.mock';

const MaintenanceRequests = () => {
  const [searchTerm, setSearchTerm] = useState('');
  
  const filteredRequests = maintenanceMockData.filter(record => 
    ...
    <div className="p-6 max-w-[1600px] mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Track, assign, and manage all corrective and preventive maintenance tasks.</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6 flex flex-col sm:flex-row gap-4 justify-between items-center">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            ...
            placeholder="Search by asset, ID, or technician..."
            value={searchTerm}
      ...
      <MaintenanceTable 
        ...
        onAssign={(record) => console.log('Assign Technician', record)}
        onComplete={(record) => console.log('Complete Request', record)}
      />
    </div>
  );
};
