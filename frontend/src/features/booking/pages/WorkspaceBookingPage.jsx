...
const WorkspaceBookingPage = () => {
  ...
  // Filter strictly for Workspace
  const workspaceBookings = filterBookings(bookings, {
    query: searchQuery,
    status: statusFilter,
    resourceType: 'Workspace',
  });

  const workspaceResources = resources.filter((r) => r.type === 'Workspace');

  return (
    <div className="space-y-6 animate-in fade-in-50 duration-200">
      <PageHeader
        title="Co-Working & Sprint Pod Workspaces"
        subtitle="Allocate hot-desking pods, collaborative project zones, quiet rooms, and team offsite pods across innovation hubs."
        breadcrumbs={[
          { label: 'AssetFlow ERP', path: '/' },
          { label: 'Reservations Hub', path: '/booking/dashboard' },
          { label: 'Workspaces & Pods' },
        ]}
        actions={
      ...
      <div className="p-4 bg-white dark:bg-sidebar-hover rounded-2xl border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xs">
        <div className="flex items-center gap-2 w-full md:w-auto">
          <BookingSearch
            ...
            placeholder="Search workspace pods, desks, teams..."
          />
      ...
      {viewMode === 'availability' ? (
        <CalendarScheduleTable resources={workspaceResources} isLoading={isLoading} />
      ) : viewMode === 'table' ? (
        <BookingTable
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {workspaceBookings.length === 0 && !isLoading && (
            <div className="col-span-full p-12 text-center text-gray-400 bg-white dark:bg-sidebar-hover rounded-xl border border-gray-200 dark:border-gray-700">
              No workspace reservations found matching your current search parameters.
            </div>
          )}
        </div>
      )}
