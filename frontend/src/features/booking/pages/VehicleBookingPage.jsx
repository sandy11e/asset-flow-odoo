...
const VehicleBookingPage = () => {
  ...
  // Filter strictly for Vehicle
  const vehicleBookings = filterBookings(bookings, {
    query: searchQuery,
    status: statusFilter,
  ...
  return (
    <div className="space-y-6 animate-in fade-in-50 duration-200">
      <PageHeader
        title="Fleet & Vehicle Dispatch Reservations"
        subtitle="Schedule transit vans, executive shuttles, and heavy-duty logistics vehicles with automated mileage & chauffeur tracking."
        breadcrumbs={[
      ...
      <div className="p-4 bg-white dark:bg-sidebar-hover rounded-2xl border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xs">
        <div className="flex items-center gap-2 w-full md:w-auto">
          <BookingSearch
            ...
            placeholder="Search fleet vans, shuttle IDs, drivers..."
          />
      ...
      {viewMode === 'availability' ? (
        <CalendarScheduleTable resources={vehicleResources} isLoading={isLoading} />
      ) : viewMode === 'table' ? (
        <BookingTable
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {vehicleBookings.length === 0 && !isLoading && (
            <div className="col-span-full p-12 text-center text-gray-400 bg-white dark:bg-sidebar-hover rounded-xl border border-gray-200 dark:border-gray-700">
              No vehicle bookings recorded matching current search parameters.
            </div>
          )}
        </div>
      )}
