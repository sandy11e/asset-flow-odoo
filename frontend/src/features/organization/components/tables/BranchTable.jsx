...
const BranchTable = ({
  ...
  const columns = [
    ...
    {
      header: 'Location',
      render: (row) => <span className="text-xs text-gray-700 dark:text-gray-300">{row.city}, {row.country}</span>,
    },
    {
      header: 'Manager',
      render: (row) => <span className="text-xs font-medium text-gray-800 dark:text-gray-200">{row.managerName}</span>,
    },
    {
      header: 'Staff',
  ...
  return (
    <div className={`overflow-x-auto ${className}`}>
      <Table
        ...
        emptyDescription="There are currently no branch facilities matching your query."
