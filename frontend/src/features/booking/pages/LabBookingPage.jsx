...
const LabBookingPage = () => {
  ...
  // Filter strictly for Lab
  const labBookings = filterBookings(bookings, {
    query: searchQuery,
    status: statusFilter,
    resourceType: 'Lab',
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in-50 duration-200">
      <PageHeader
        title="Specialized Cleanroom & Research Lab Reservations"
        subtitle="Manage access to cleanroom facilities, AI robotics testing pods, laser spectroscopy labs, and chemical calibration bays."
        breadcrumbs={[
      ...
      <div className="p-4 bg-white dark:bg-sidebar-hover rounded-2xl border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xs">
        <div className="flex items-center gap-2 w-full md:w-auto">
          <BookingSearch
            ...
            placeholder="Search lab rooms, cleanrooms, researchers..."
          />
      ...
      {viewMode === 'availability' ? (
        <CalendarScheduleTable resources={labResources} isLoading={isLoading} />
      ) : viewMode === 'table' ? (
        <BookingTable
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {labBookings.length === 0 && !isLoading && (
            <div className="col-span-full p-12 text-center text-gray-400 bg-white dark:bg-sidebar-hover rounded-xl border border-gray-200 dark:border-gray-700">
              No specialized lab reservations found matching your current search criteria.
            </div>
          )}
        </div>
      )}
